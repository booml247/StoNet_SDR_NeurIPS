...
for cross_validate_index in range(1):
    # load data
    import pickle
    PATH = "./result/"+data_name+"/stonet/reduce_dim_"+str(reduce_dim)+"/cross_0/hidden_layer_reduce_dim_"+str(reduce_dim)+".txt"
    f = open(PATH, 'rb')
    last_hidden_layer_train, last_hidden_layer_test, _ = pickle.load(f)
    f.close()
